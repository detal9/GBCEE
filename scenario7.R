...
included_BAC = matrix(, nrow = nrep, ncol = p + 1);
...
for(n in c(200, 1000))
{
 ...
 for(i in 1:nrep)
 {
  ...
  results = bac(data = ds, exposure = "X", outcome = "Y", confounders = varlist, interactors = "U3",
  ...
  full = lm(Y~X+U+X:U[,3]);
  Q1_0 = cbind(1, 1, U, U[,3])%*%coef(full);
  Q0_0 = cbind(1, 0, U, 0)%*%coef(full);
  coefs_full[i] = mean(Q1_0 - Q0_0);
  sd_full[i] = NA;
  CP_full[i] = NA;

  ### Target model
  target = lm(Y~X+U[,3:5]+X:U[,3]);
  Q1_0 = cbind(1, 1, U[,3:5], U[,3])%*%coef(target);
  Q0_0 = cbind(1, 0, U[,3:5], 0)%*%coef(target);
  coefs_target[i] = mean(Q1_0 - Q0_0);
  sd_target[i] = NA;
  CP_target[i] = NA;
  ...
  Q1_0 = cbind(1, 1, U, U[,3])%*%coef(full);
  Q0_0 = cbind(1, 0, U, 0)%*%coef(full);
  QX_0 = cbind(1, X, U, X*U[,3])%*%coef(full);
  ...
  ### Target adjusted model - AIPTW
  fullX = glm(X~U[,3:5], family = "binomial");
  pX1 = plogis(cbind(1, U[,3:5])%*%coef(fullX));
  bounds = quantile(pX1, c(0.01, 0.99)); 
  pX1 = pmin(pX1, bounds[2]);
  pX1 = pmax(pX1, bounds[1]);
  H1 = X/pX1; H0 = (1 - X)/(1 - pX1);
  Q1_0 = cbind(1, 1, U[,3:5], U[,3])%*%coef(target);
  Q0_0 = cbind(1, 0, U[,3:5], 0)%*%coef(target);
  QX_0 = cbind(1, X, U[,3:5], X*U[,3])%*%coef(target);
 ...
 ### Record data
 file.name = paste("results_scenario7_corrections_n",n,"_",gsub("-","",Sys.Date()),".Rdata", sep="");
