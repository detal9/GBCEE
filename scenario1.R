...
for(n in c(200, 1000))
{
 ...
 for(i in 1:nrep)
 {
  ...
  pX1 = pmax(pX1, bounds[1]);
  H1 = X/pX1; H0 = (1 - X)/(1 - pX1);
  Q1_0 = cbind(1, 1, U[,1:10])%*%coef(target);
  Q0_0 = cbind(1, 0, U[,1:10])%*%coef(target);
  QX_0 = cbind(1, X, U[,1:10])%*%coef(target);
 ...
 ### Record data
 file.name = paste("results_scenario1_n",n,"_",gsub("-","",Sys.Date()),".Rdata", sep="");
