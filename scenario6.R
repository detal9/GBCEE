...
for(n in c(200, 1000))
{
 ...
 for(i in 1:nrep)
 {
  ...
  ### Target model - AIPTW
  targetX = lm(X~U[,1:25]);
  pX1 = plogis(cbind(1, U[,1:25])%*%coef(targetX));
  bounds = quantile(pX1, c(0.01, 0.99)); 
  pX1 = pmin(pX1, bounds[2]);
  pX1 = pmax(pX1, bounds[1]);
  H1 = X/pX1; H0 = (1 - X)/(1 - pX1);
  Q1_0 = cbind(1, 1, U[,1:25])%*%coef(target);
  Q0_0 = cbind(1, 0, U[,1:25])%*%coef(target);
  QX_0 = cbind(1, X, U[,1:25])%*%coef(target);
 ...
 ### Record data
 file.name = paste("results_scenario6_n",n,"_",gsub("-","",Sys.Date()),".Rdata", sep="");
