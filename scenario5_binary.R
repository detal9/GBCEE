...
#Y0 = rbinom(n, size = 1, p = plogis(trueB*0 + 0.5*U[,1] + 0.5*U[,2] + 0.1*U[,3]));
#Y1 = rbinom(n, size = 1, p = plogis(trueB*1 + 0.5*U[,1] + 0.5*U[,2] + 0.1*U[,3]));
...
for(n in c(1000))
{
 ...
 for(i in 1:nrep)
 {
  print(data.frame(i/nrep*100, Sys.time()));

  ### simulate data
  U = rmvnorm(n = n, mean = rep(1, p), sigma = diag(1-rho, nrow = p) + matrix(rho, nrow = p, ncol = p));
  U2 = t(apply(U, 1, function(x){c(x%*%t(x))}));
  X = rbinom(n, size = 1, p = plogis(-5 + U[,3] + U[,4] + U[,5] + rowSums(U2)));
  Y = rbinom(n, size = 1, p = plogis(trueB*X + 0.5*U[,1] + 0.5*U[,2] + 0.1*U[,3]));
  ...
  ### Target model
  target = glm(Y~X+U[,1:3], family = "binomial");
  ...
  ### Target model - AIPTW
  targetX = glm(X~U[,1:3], family = "binomial");
  pX1 = plogis(cbind(1, U[,1:3])%*%coef(targetX));
  bounds = quantile(pX1, c(0.01, 0.99)); 
  pX1 = pmin(pX1, bounds[2]);
  pX1 = pmax(pX1, bounds[1]);
  H1 = X/pX1; H0 = (1 - X)/(1 - pX1);
  Q1_0 = plogis(cbind(1, 1, U[,1:3])%*%coef(target));
  Q0_0 = plogis(cbind(1, 0, U[,1:3])%*%coef(target));
  QX_0 = plogis(cbind(1, X, U[,1:3])%*%coef(target));
 ...
 ### Record data
 file.name = paste("results_scenario5B_n",n,"_",gsub("-","",Sys.Date()),".Rdata", sep="");
