...
#Y0 = rbinom(n, size = 1, p = plogis(-5 + trueB*0 + U[,3] + U[,4] + U[,5] + rowSums(U2)));
#Y1 = rbinom(n, size = 1, p = plogis(-5 + trueB*1 + U[,3] + U[,4] + U[,5] + rowSums(U2)));
...
n = 1000;
{
 ...
 for(i in 1:nrep)
 {
  ...
  U2 = t(apply(U, 1, function(x){c(x%*%t(x))}));
  X = rbinom(n, size = 1, p = plogis(0.5*U[,1] + 0.5*U[,2] + 0.1*U[,3]));
  Y = rbinom(n, size = 1, p = plogis(-5 + trueB*X + U[,3] + U[,4] + U[,5] + rowSums(U2)));
 ...
 ### Record data
 file.name = paste("results_scenario4Binary_n",n,"_",gsub("-","",Sys.Date()),".Rdata", sep="");
