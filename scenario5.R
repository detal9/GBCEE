...
tryCatch({
  ...
  CP_GBCEE_TMLEv2[i] = trueATE > results$beta[1] - 1.96*results$stderr[1] & trueATE < results$beta[1] + 1.96*results$stderr[1];
   ...
   coefs_GBCEE_TMLEv2[i] = NA;
   sd_GBCEE_TMLEv2[i] = NA;
   included_GBCEEv2[i, ] = NA;
   included_GBCEEXv2[i, ] = NA;
   CP_GBCEE_TMLEv2[i] = NA;
  });
  ...
  pX1 = pmax(pX1, bounds[1]);
  H1 = X/pX1; H0 = (1 - X)/(1 - pX1);
  Q1_0 = cbind(1, 1, U[,1:3])%*%coef(target);
  Q0_0 = cbind(1, 0, U[,1:3])%*%coef(target);
  QX_0 = cbind(1, X, U[,1:3])%*%coef(target);
 ...
 ### Record data
 file.name = paste("results_scenario5_n",n,"_",gsub("-","",Sys.Date()),".Rdata", sep="");
